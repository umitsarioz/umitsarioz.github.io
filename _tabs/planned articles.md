---
icon: fa-solid fa-pencil
order: 4
---

> I plan to create blog content around the following topics. New additions and deletions will be made here over time.

- [ ] From Scratch Algorithms
  - [ ] Decision Trees
  - [ ] Artificial Neural Networks
- [ ] Neural Networks
  - [ ] Optimization Algorithms in Neural Networks
  - [ ] Backpropagation & Forward Pass Step in Neural Networks
  - [ ] Activation Functions in Neural Networks
- [ ] PyTorch
  - [ ] Install & CUDA Install
  - [ ] Basic Examples & Tutorial
  - [ ] Multi-Class Classification Tutorial
  - [ ] Binary Classification Tutorial
  - [ ] Regression Tutorial
- [ ] Natural Language Processing
  - [ ] Chatbots, QA & Summarization
  - [ ] Transformer Architecture & Paper
  - [ ] From Scratch Transformer Architecture
  - [ ] Large Language Models & Comparison 
  - [ ] Embedding, Tokenization, Quantization
  - [ ] Finetuning & Retrieved Augmented Generation
- [ ] Generative AI
- [ ] Clustering
  - [ ] Evaluation Methods
  - [ ] Algorithms
- [ ] Graph Problems
  - [ ] Graph Libraries
  - [ ] Graph Algorithms & Implementations
- [ ] Time Series
  - [ ] Time Series Problems Behaviour & Tricks
  - [ ] Time Series Algorithms/Models
    - [ ] ARIMA
    - [ ] SARIMA
    - [ ] Neural Networks
    - [ ] Prophet
- [ ] AI Impact on Non-scientific areas
- [ ] Anomaly Detection & Algorithms
- [ ] Optimization Algorithms
  - [ ] Price Optimization
  - [ ] Stock Optimization
  - [ ] Traffic Optimization
- [ ] Recommendation Algorithms
  - [ ] Statistical methods
  - [ ] Collobaritive filtering
- [ ] Data Science Packages: Installation & Basic Tutorials
  - [ ] NumPy 
  - [ ] Pandas
  - [ ] Scikit-Learn
  - [ ] Matplotlib
  - [ ] Seaborn
  - [ ] Plotly
- [ ] Database designs & architectures
- [ ] Big Data Tools : Installation & Usages
  - [ ] Docker
  - [ ] Kubernetes
  - [ ] PostgreSQL
  - [ ] MongoDB
  - [ ] Apache Cassandra
  - [ ] Apache Hadoop
  - [ ] Apache Kafka
  - [ ] Apache Flink
  - [ ] Apache Airflow
  - [ ] Apache Sqoop

